...
create_images <- function(non_empty_sleepopties, full_output_directory, tekst_itemnummer, max_chars_per_line) {
  ...
  for (var_name in names(non_empty_sleepopties)) {
    ...
    output_filename <- file.path(full_output_directory, paste0(tekst_titel, "_", tekst_itemnummer, "_", var_name, ".jpg")) # Use full_output_directory here
...
for (i in seq_len(nrow(data))) {
  if (is.na(data$gegenereerd[i]) || nchar(data$gegenereerd[i]) == 0) {
    ...
    create_images(non_empty_sleepopties, full_output_directory, tekst_itemnummer, max_chars_per_line)  # Updated function call
...
# Update 'gegenereerd' column in the Excel file
